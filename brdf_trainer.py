...
class BRDFTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Compute surface normal (assuming sphere at origin with radius 0.2)
        normal = pos / torch.norm(pos, dim=-1, keepdim=True)  # [N, 3]
        
        # Evaluate BRDF using the material model
        brdf_pred, _ = self.material.eval_brdf(
            gt_params=gt_params,  # Add batch dimension
            pos=pos,
            wi=wi,
            wo=wo,
            normal=normal
        ...
        # Evaluate ground truth BRDF using gt_material
        with torch.no_grad():
            brdf_gt, _ = self.gt_material.eval_brdf(
                params=gt_params,  # Add batch dimension
                pos=pos,
                wi=wi,
                wo=wo,
                normal=normal
        ...
        # Compute PSNR-like metric for BRDF values
        psnr_loss = NF.mse_loss(brdf_pred, brdf_gt)
        psnr = -10.0 * torch.log10(psnr_loss.clamp_min(1e-8))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Render step logic
        rays, gt_params = batch['rays'], batch['gt_params']
        rgbs, brdf = self.renderer.render(emitter, rays, self.cfg.renderer.spp.train, None, None)

        if self.gt_folder is None:
            with torch.no_grad():
                rgbs_gt, brdf_gt = self.gt_renderer.render(emitter, rays, self.cfg.renderer.spp.train, gt_params, None)
        else:
            rgbs_gt = batch['rgbs']
        ...
        self.log('val/loss', loss)
        self.log('val/psnr', psnr)   
        self.log('val/brdf_loss', brdf_loss)
        return
