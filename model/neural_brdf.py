...
from utils.old_moe import MoE,MLP
...
def load_latent_texture_from_checkpoint(checkpoint_path, device='cuda'):
    ...
    latent_part=latent_texture[:, :, :, :]
    #for i in range(0,2000,10):
    #    print("latent_part1", latent_part[:,:8,i,1000])
    #    print("latent_par21", latent_part[:,:8,1000,i])
...
class AnisotropicLatentTexturedModel(LightningModule):
    ...
    def __init__(self, cfg):
        ...
        self.latent_texture = load_latent_texture_from_checkpoint(cfg.latent_texture_path2)
        # gaussian blur parameters
        self.Gaussian_blur = cfg.Gaussian_blur
        self.blur_sigma0 = 8.0
        self.blur_half_life = 3333
        ...
        if self.different_decoder:
            ...
            #self.mlp = nn.Sequential(*layers)
            self.mlp = MLP(input_dim, cfg.output_channels, 32)
    ...
    def forward(self, enc_dir, latent=None, channel=None):
        ...
        Returns:
            brdf: Bx1 BRDF values
        """   
        #latent = self.pre_moe_mlp(latent)
        if self.different_decoder:
            if channel == 'r':
...
class MoELatentTexturedModel(LightningModule):
    """ MLP-based BRDF class with 2D texture latent grids """
    def __init__(self, cfg):
        super().__init__()
        ...
        #self.latent_texture = nn.Parameter(latent_init)
        self.latent_texture_sup = load_latent_texture_from_checkpoint(cfg.latent_texture_path1)
        self.latent_texture = load_latent_texture_from_checkpoint(cfg.latent_texture_path1)
        #self.latent_texture=self.latent_texture[:, 8:, :, :]
        '''
        self.latent_texture = torch.cat([self.latent_texture[:, :8, :, :], self.latent_texture[:, 24:, :, :]], dim=1)
        
        self.latent_texture[..., -6:-3, :, :] = self.latent_texture_sup[..., -6:-3, :, :]
        self.latent_texture[..., -3:, :, :] = self.latent_texture_sup[..., -3:, :, :]
        self.latent_texture[:, :8, :, :]=torch.randn(1, 8, self.texture_resolution, self.texture_resolution)
        '''
        print("latent_texture", self.latent_texture.shape)
        # gaussian blur parameters
        self.Gaussian_blur = cfg.Gaussian_blur
        self.blur_sigma0 = 8.0
        self.blur_half_life = 3333
        ...
        self.moe_k = getattr(cfg.moe, 'moe_k', 2)
        self.moe_hidden_size = getattr(cfg.moe, 'moe_hidden_size', 32)
        ...
        if self.different_decoder:
            ...
            #self.moe = MLP(input_dim, cfg.output_channels, self.moe_hidden_size)
...
class MoELatentTexturedModel(LightningModule):
    ...
    def __init__(self, cfg):
        ...
        # Add latent dimension to input
        input_dim = encoded_input_dim + self.latent_dim 
