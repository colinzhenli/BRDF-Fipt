...
Detected warnings:
  - CATASTROPHIC_TRANS : all 10 top-per-image translation errors > 20 mm
    (Umeyama alignment has likely failed; even the 16 mm per-image filter
    in shape_matching.py will not rescue this material)
...
def detect_quality_warnings(folder_path: str) -> List[Dict[str, str]]:
    ...
    if errs_mm is not None and len(errs_mm) == 10:
        if all(e > CATASTROPHIC_TRANS_MM for e in errs_mm):
            warnings.append({
                "code": "CATASTROPHIC_TRANS",
                "msg": (f"all 10 top translation errors > "
