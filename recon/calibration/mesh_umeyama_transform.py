...
def load_matched_robot_poses(scan_log_path, transforms_json_path, tol=1e-3):
    ...
    for i, frame in enumerate(tjson["frames"]):
        ...
        T[:3, 3] = np.array(entry["position"]) / 1000.0
...
def parse_colmap_images_txt(images_txt_path):
    ...
    while i < len(lines):
        ...
        T[:3, :3] = R
        T[:3,  3] = t
        cam_c2w_dict[image_name] = _cv_to_gl(T)
        i += 2  # skip the 2D point line
    return cam_c2w_dict

def parse_colmap_images_txt_neuralangelo(images_txt_path, images):
    """
    Parse COLMAP images.txt and return a dictionary of image_name → 4x4 cam-to-world transform.
    """
    cam_c2w_dict = {}
...
def load_matched_robot_poses_colmap_images_txt(scan_log_path, images_txt_path, images, tol=1e-3):
    """
    ...
    colmap_c2w_dict = parse_colmap_images_txt_neuralangelo(images_txt_path, images)
...
def estimate_world2base(scan_log_path, images_txt_path, images,
    ...
    robot_T, cam_c2w, _ = load_matched_robot_poses_colmap_images_txt(
        scan_log_path, images_txt_path, images, phi_theta_tol
    )
    ...
    T_BW[:3, :3] = s * R
    T_BW[:3,  3] = t
    # Manually lift up z axis by 0.12m
    # T_BW[2, 3] += 0.12
    return T_BW
...
# -----------------------------------------------------------------------------#
def main(scan_log_path, images_txt_path, images, mesh_path, tol=1e-2):
    ...
    t_c2g = np.array([36.68630125, -24.61733549, 27.64501449]) / 1000.0

    T_BW = estimate_world2base(scan_log_path, images_txt_path, images, R_c2g, t_c2g, tol)
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--transforms_json_path", type=str, required=True,
    #                     help="Path to COLMAP/NeRF transforms.json file")
    # parser.add_argument("--images_txt_path", type=str, required=True,
    ...
    parser.add_argument("--tol", type=float, default=0.01,
                        help="Tolerance for phi/theta matching (default: 0.01)")
    
    args = parser.parse_args()
    images_txt_path = os.path.join(args.model_path, "images.txt")
    
    cameras, images, points3D = read_model(args.model_path, ext=".bin")
    main(args.scan_log_path, images_txt_path, images, args.mesh_path, args.tol)
# 