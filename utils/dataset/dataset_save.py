...
class MERLBRDFDataset(Dataset):
    """
    Dataset for loading MERL BRDF database materials.
    
    The MERL BRDF database contains densely measured BRDFs for 100 different materials.
    Each material is stored in a binary file with the following format:
    ...
    - Invalid/unmeasured samples are marked with -1.0
    
    The BRDF is parameterized by Rusinkiewicz coordinates:
    - theta_h: half-angle between wi and wo (0 to pi/2)
    - theta_d: difference angle (0 to pi/2)
    - phi_d: azimuthal difference (0 to 2*pi)
    
    Standard MERL dimensions are 90 x 90 x 180 = 1,458,000 samples per material.
    
    Args:
        data_folder: Path to folder containing .binary BRDF files
        material_names: Optional list of specific material names to load (without .binary extension)
                       If None, loads all .binary files in the folder
        normalize: Whether to normalize BRDF values (default: True)
        filter_invalid: Whether to filter out invalid samples (marked as -1.0) (default: True)
        device: Device to store tensors on (default: 'cuda' if available, else 'cpu')
    """
    
    def __init__(self, data_folder, material_names=None, normalize=True, filter_invalid=True, device=None):
        self.data_folder = Path(data_folder)
        self.normalize = normalize
        self.filter_invalid = filter_invalid
        self.device = device if device is not None else ('cuda' if torch.cuda.is_available() else 'cpu')
        
        # Discover available BRDF files
        if material_names is not None:
            self.brdf_files = [self.data_folder / f"{name}.binary" for name in material_names]
            # Verify files exist
            self.brdf_files = [f for f in self.brdf_files if f.exists()]
        else:
            self.brdf_files = sorted(self.data_folder.glob("*.binary"))
        
        if len(self.brdf_files) == 0:
            raise ValueError(f"No BRDF files found in {data_folder}")
        ...
        print(f"Found {len(self.brdf_files)} BRDF materials")
        
        # Load all BRDF data
        self.materials = []
        self.material_names = []
        
        for brdf_file in tqdm(self.brdf_files, desc="Loading BRDF materials"):
            material_name = brdf_file.stem  # filename without extension
            brdf_data = self._load_brdf_file(brdf_file)
            
            if brdf_data is not None:
                self.materials.append(brdf_data)
                self.material_names.append(material_name)
        
        print(f"Successfully loaded {len(self.materials)} materials")
        print(f"{'='*60}\n")
    
    def _load_brdf_file(self, filepath):
        """
        Load a single MERL BRDF binary file.
        
        Returns:
            dict with keys:
                - 'rgb': (N, 3) tensor of RGB reflectance values
                - 'coords': (N, 3) tensor of (theta_h, theta_d, phi_d) coordinates
                - 'dims': tuple of (n_theta_h, n_theta_d, n_phi_d)
                - 'valid_mask': (N,) boolean tensor indicating valid samples
        """
        try:
            with open(filepath, 'rb') as f:
                ...
                if len(brdf_data) != n_samples * 3:
                    print(f"Warning: Expected {n_samples * 3} values, got {len(brdf_data)} in {filepath.name}")
                    return None
                
                # Reshape to (n_samples, 3) for RGB
                brdf_rgb = brdf_data.reshape(n_samples, 3)
                
                # Generate coordinate grid
                # MERL BRDF uses Rusinkiewicz parameterization
                # theta_h: [0, pi/2], theta_d: [0, pi/2], phi_d: [0, 2*pi)
                theta_h_vals = np.linspace(0, np.pi / 2, n_theta_h)
                theta_d_vals = np.linspace(0, np.pi / 2, n_theta_d)
                phi_d_vals = np.linspace(0, 2 * np.pi, n_phi_d, endpoint=False)
                
                # Create meshgrid and flatten
                theta_h_grid, theta_d_grid, phi_d_grid = np.meshgrid(
                    theta_h_vals, theta_d_vals, phi_d_vals, indexing='ij'
                )
                coords = np.stack([
                    theta_h_grid.ravel(),
                    theta_d_grid.ravel(),
                ...
                brdf_rgb = torch.from_numpy(brdf_rgb).float().to(self.device)
                coords = torch.from_numpy(coords).float().to(self.device)
                
                # Identify valid samples (MERL uses -1.0 for invalid/unmeasured)
                valid_mask = (brdf_rgb >= 0).all(dim=1)
                
                # Filter invalid samples if requested
                if self.filter_invalid:
                    brdf_rgb = brdf_rgb[valid_mask]
                    coords = coords[valid_mask]
                    # Update valid_mask to all True after filtering
                    valid_mask = torch.ones(len(brdf_rgb), dtype=torch.bool)
                
                # Normalize BRDF values if requested
                if self.normalize and len(brdf_rgb) > 0:
                    # Clip extreme values and normalize to [0, 1] range
                    # Use 99th percentile to avoid outliers
                    max_val = torch.quantile(brdf_rgb[brdf_rgb > 0], 0.99) if (brdf_rgb > 0).any() else 1.0
                    brdf_rgb = torch.clamp(brdf_rgb / max_val, 0, 1)
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            dict with keys:
                ...
                - 'rgb': (N, 3) tensor of RGB reflectance values
                - 'coords': (N, 3) tensor of (theta_h, theta_d, phi_d) coordinates in radians
                - 'dims': tuple of original dimensions (n_theta_h, n_theta_d, n_phi_d)
        """
        material_data = self.materials[idx]
        
        return {
            'material_name': self.material_names[idx],
            'material_id': idx,
            'rgb': material_data['rgb'],
            'coords': material_data['coords'],
            'dims': material_data['dims'],
        }
    
    def get_material_by_name(self, name):
        """Get BRDF data for a specific material by name."""
        if name in self.material_names:
            idx = self.material_names.index(name)
            return self[idx]
        else:
            raise ValueError(f"Material '{name}' not found. Available: {self.material_names}")
    
    def sample_random_directions(self, material_idx, n_samples):
        """
        Sample random BRDF values from a material (GPU accelerated).
        ...
        Returns:
            dict with 'rgb' and 'coords' tensors of shape (n_samples, 3) on device
        """
        material_data = self.materials[material_idx]
        n_available = len(material_data['rgb'])
        
        # GPU-accelerated random sampling
        if n_samples > n_available:
            # Sample with replacement if needed
            indices = torch.randint(0, n_available, (n_samples,), device=self.device)
        else:
            # Sample without replacement
            indices = torch.randperm(n_available, device=self.device)[:n_samples]
        
        return {
            'rgb': material_data['rgb'][indices],
            'coords': material_data['coords'][indices],
            'material_id': material_idx,
            'material_name': self.material_names[material_idx],
        }


class MERLBRDFIterableDataset(IterableDataset):
    """
    Iterable dataset for MERL BRDF data that yields random samples (GPU accelerated).
    
    This dataset continuously yields batches of random BRDF samples from the loaded materials.
    All data is loaded to GPU during initialization for fast sampling.
    Useful for training neural BRDF models.
    
    Args:
        data_folder: Path to folder containing .binary BRDF files
        batch_size: Number of samples per batch
        material_names: Optional list of specific materials to load
        normalize: Whether to normalize BRDF values
        filter_invalid: Whether to filter out invalid samples
        samples_per_material: Number of samples to draw from each material per iteration
        device: Device to store tensors on (default: 'cuda' if available, else 'cpu')
    """
    
    def __init__(self, data_folder, batch_size=1024, material_names=None, 
                 normalize=True, filter_invalid=True, samples_per_material=None, device=None):
        self.base_dataset = MERLBRDFDataset(
        ...
        self.samples_per_material = samples_per_material or (batch_size // len(self.base_dataset))
        self.device = self.base_dataset.device
    
    def __iter__(self):
        """Infinite iterator yielding random BRDF samples (GPU accelerated)."""
        while True:
            batch_rgb = []
            batch_coords = []
            batch_material_ids = []
            
            # Sample from each material (all on GPU)
            for material_idx in range(len(self.base_dataset)):
                sample_data = self.base_dataset.sample_random_directions(
                    material_idx, self.samples_per_material
                )
                batch_rgb.append(sample_data['rgb'])
                batch_coords.append(sample_data['coords'])
                batch_material_ids.append(
                    torch.full((self.samples_per_material,), material_idx, dtype=torch.long, device=self.device)
                )
            
            # Concatenate and shuffle (all on GPU)
            batch_rgb = torch.cat(batch_rgb, dim=0)
            batch_coords = torch.cat(batch_coords, dim=0)
            batch_material_ids = torch.cat(batch_material_ids, dim=0)
            
            # Randomly permute (GPU accelerated)
            perm = torch.randperm(len(batch_rgb), device=self.device)
            batch_rgb = batch_rgb[perm][:self.batch_size]
            batch_coords = batch_coords[perm][:self.batch_size]
            batch_material_ids = batch_material_ids[perm][:self.batch_size]
