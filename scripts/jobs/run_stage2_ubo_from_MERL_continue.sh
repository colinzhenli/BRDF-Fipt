...
# Continue (resume FULL training state) a stage-2 UBO "from-MERL" run from its
...
# (freeze_decoder=True, factor_init etc.); only experiment_name, continue_training
# and ckpt_path change.
#
# NOTE: the original (non-continue) script names the run with a "Theia2_" prefix,
# but the Leonardo-copied runs on disk are named WITHOUT it
# (Stage-2_UBO_<btf>_from-MERL_Grazing-angle-init-0.1_run_1). Set ORIG_EXP to the
...
ORIG_EXP=Stage-2_UBO_${BTF_NAME}_from-MERL_Grazing-angle-init-0.1_run_1   # <-- verify against disk
